...
export function validateDraft(
  ...
  const lessonSlugs = new Set(lessons.map((entry) => entry.slug));
  for (const prerequisite of draft.preRequisitos) {
    ...
    if (
      ...
      lessons.find((entry) => entry.slug === prerequisite)?.status !== "publicado"
    ) {
      issues.push(
        issue(
          "DRAFT_PREREQUISITE",
          `Aula publicada não deve depender do rascunho ${prerequisite}.`,
          "preRequisitos",
        ),
      );
    }
